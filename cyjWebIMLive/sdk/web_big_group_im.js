...
(function(WebBigGroupIM) {
	...
	var getApiUrl = function(srvName,cmd, cbOk, cbErr) {
                ...
                url=host+'/'+SRV_NAME_VER[srvName]+'/'+srvName+'/'+cmd+'?websdkappid='+SDK.APPID+'&v='+SDK.VERSION+'&sdkappid='+ctx.sdkAppID+'&appidat3rd='+ctx.appIDAt3rd;
                if(isLogin()){
                    if(cmd=='login'){
                ...
                url+='&apn='+ctx.apn+'&reqtime='+unixtime();
		return url;
	};
	...
	var proto_sendMsg = function(msg, cbOk, cbErr) {
		...
		if (!checkLogin(cbErr,true)) return;
                ...
                //Array	选填	初始群成员列表，元素必须是字符串类型，最多500个。成员信息字段详情参见：群成员资料。
	...
	WebBigGroupIM.logout = function(cbOk,cbErr) {return proto_logout(cbOk, cbErr);};
        ...
        //资料接口
        //获取个人资料接口，可用于搜索用户
        WebBigGroupIM.getProfilePortrait = function(options,cbOk, cbErr) {return proto_getProfilePortrait(options,cbOk, cbErr);};
        //设置个人资料
        WebBigGroupIM.setProfilePortrait = function(options,cbOk, cbErr) {return proto_setProfilePortrait(options,cbOk, cbErr);};
